...
function tree_component () {
	...
	return {
		...
		create : function (type, obj, data, icon ) {
			...
			if(this.settings.rules.metadata) {
				jQuery.metadata.setType("attr", this.settings.rules.metadata);
				jQueryli.attr(this.settings.rules.metadata, "type: '" + t + "'");
			}
			else {
				jQueryli.attr(this.settings.rules.type_attr, t)
			}
			...
			if(this.settings.languages.length) {
				for(i = 0; i < this.settings.languages.length; i++) {
					...
					jQueryli.append("<a href='#'" + ( icn.length ? " style='background-image:url(\"" + icn + "\");' " : " ") + "class='" + this.settings.languages[i] + "'>" + val + "</a>");
				}
			}
			else { jQueryli.append("<a href='#'" + ( icn.length ? " style='background-image:url(\"" + icn + "\");' " : " ") + ">" + (data || this.settings.lang.new_node || "New folder") + "</a>"); }
			jQueryli.addClass("leaf");
			if(this.settings.rules.createat == "top" || obj.children("ul").size() == 0) {
				this.moved(jQueryli,obj.children("a:eq(0)"),"inside", true);
			}
			else {
				this.moved(jQueryli,obj.children("ul:eq(0)").children("li:last").children("a:eq(0)"),"after",true);
			}
		...
		moved : function (what, where, how, is_new, is_copy) {
			...
			switch(how) {
				...
				case "inside":
					if(this.settings.data.async) {
						var obj = this.get_node(jQuerywhere);
						if(obj.hasClass("closed")) {
							var _this = this;
